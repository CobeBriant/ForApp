...
public class LoginController {
	...
	@ResponseBody
	public String doLogin1(@RequestBody String requestBdy) {		
		try {
			com.alibaba.fastjson.JSONObject params =com.alibaba.fastjson.JSONObject.parseObject(requestBdy);
//		System.out.println(requestBdy);  
//		System.out.println(params.toString());
		...
		JSONObject returnJson = new JSONObject();
			if ((!DataUtil.isValid(username, password))) {
				returnJson.put("code", "300");
				returnJson.put("message", "用户名或密码无效");
			...
			if (role == 1) {
				...
				if (student == null) {
					returnJson.put("code", "300");
					returnJson.put("message", "用户名或密码错误");
					return returnJson.toString();
				}
				student.setPassword(password);
			    returnJson.put("code", "200");
				returnJson.put("message", "登陆成功");
				com.alibaba.fastjson.JSONObject studentJson = (com.alibaba.fastjson.JSONObject) com.alibaba.fastjson.JSON.toJSON(student);
				returnJson.put("student", studentJson);
				return returnJson.toString();
			}
			returnJson.put("code", "0");
			returnJson.put("message", "非学生账号！");
			return returnJson.toString();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void rand(String verify, HttpServletResponse response, HttpSession session) {
		JSON json = new exam.util.json.JSONObject();
